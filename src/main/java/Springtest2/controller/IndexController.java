...
public class IndexController {//class시작
    ...
    public String save( HttpServletRequest request ){
        ...
        //2.엔티티를 db에 저장을 해주는 save 메소드 사용
    ...
    //3. main.js내 호출 메소드가 요청하는 URL 정의
        //@ResposeBody 뺀이유 :HttpServletResponse 사용
    @GetMapping("/getlist")
    ...
    @GetMapping("/update")
    @ResponseBody
    @Transactional  // 트랜잭션 : 일 단위 [결과: commit 혹은 rollback]///?선택javax?or//application.properties 한글깨짐
    public String update( HttpServletRequest request ){
        //0. 변수요청
         ...
         //1. pk값을 이용한 엔티티 찾기
        Optional<TestEntity> optionalTestEntity = testRepository.findById( no );
            //Optional 클래스 : 제네릭 클래스의 객체를 저장[만약에 null이면 null 저장]
        //2. Optional객체내 엔티티 빼오기 [ Optional 객체내 실제 entity 가져오기]
        TestEntity entity = optionalTestEntity.get();
        //3. 수정
